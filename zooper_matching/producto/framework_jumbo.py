...
from producto.models import ProductoZooper, ProductoJumbo
from common import utils_bdd as bdd

class ManagerProductoJumbo(object):
    """
...
#    def __init__(self, titulo):
    ...
    def obtenerProducto(id):
        """
            Función que obtiene el ProductoJumbo
            Param:
                id: id del producto
            Return:
                ProductoJumbo
        ...
        """
        try:
            return ProductoJumbo.objects.get(id=id)
        except Exception,e:
            return None
    ...
    def obtenerProductos(self, **kwargs):
        """
            ...
            Return:
        """
        productos = ProductoJumbo.objects.all()
        if 'ids' in kwargs:
    ...
    def siguienteProducto(self, cantidad=1):
        ...
        """
        siguientes_producto = ProductoJumbo.objects.filter(productozooper__isnull=True)[:cantidad]
    ...
    def coincidenciasZooper(self, productozooper_id):
        ...
        q_soundex = """SELECT * FROM producto_productojumbo p WHERE soundex(p.marca) = soundex('{}');""".format(productozooper.marca)
        ...
        for producto_otro_supermercado in resultados_soundex:
            ...
            for palabra in lista_palabras_producto:
                productos_puntaje = ProductoJumbo.objects.filter(
    ...
    def desvincularProducto(self, productozooper_id):
        ...
        """
        productos = self.obtenerProducto(productozooper_id=productozooper_id)
