...
class LoginSpider(BaseSpider):
    ...
    def parse_paginalogin(self, response):
        ...
        if "authentication failed" in response.body:
            ...
            while fecha_actual >= fecha_final:
                ...
                archivo_registro = 'cotmedia_files/cotmedia__'+fecha_url+'.txt'
                # print archivo_registro
                # print unicode(os.path.exists(archivo_registro))
                # print fecha_url ,
                if not os.path.exists(archivo_registro):
                    # print 'existe'
                    yield scrapy.Request(url="http://www.cotmedia.cl/cotmedia2/admin/pauta.php?dia="+fecha_url,callback=self.parse_paginapauta)
                # else:
                    # print '------'
    ...
    def parse_paginapauta(self, response):
        ...
        nombre_archivo = 'cotmedia__'+response.url.split('=')[1]+'.txt'
        print '|'*25
        print '|'*25
        print 'V'*25
        ...
        while i < len(filas_tabla) - 1:
            ...
            aux_fila['fecha_publicacion'] = response.url.split('=')[1]+' 00:00:00'
